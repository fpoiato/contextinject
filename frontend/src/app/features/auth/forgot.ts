...
@Component({
  ...
  template: `
    <section class="mx-auto flex min-h-[70vh] max-w-md flex-col justify-center px-5 py-16">
      ...
      <p class="mt-2 text-sm text-slate-400">We’ll email a code if that address has an account.</p>
      ...
      <p class="mt-6 text-sm text-slate-400">
        <a routerLink="/login" class="font-medium text-cyan-300 hover:text-cyan-200">Back to sign in</a>
      </p>
    </section>
  `,
})
export class Forgot {
  ...
  async submit(): Promise<void> {
    ...
    try {
      ...
      await this.router.navigate(["/reset"], { queryParams: { email: this.email } });
    } catch (err) {
      this.error.set(err instanceof Error ? err.message : "Could not start the reset");
    } finally {
      this.busy.set(false);
    }
  }
