...
- **Auth:** Cognito User Pool (`easyrag-users`) **sem Hosted UI**. O Angular tem telas próprias (`/login`, `/signup`, `/verify`, `/forgot`, `/reset`). Uma Lambda de auth chama as Admin APIs do Cognito (`AdminInitiateAuth`, signup/confirm/forgot) e o **API Gateway HTTP** publica `POST /auth/*`. O CloudFront de `api.easyrag.fpoiato.com` encaminha `/auth*` para esse API. Códigos de verificação e reset saem pelo Amazon SES do domínio `fpoiato.com` (`noreply@fpoiato.com`), não pelo `verificationemail.com` padrão do Cognito. O front guarda o ID token e envia `Authorization: Bearer`; a API de chat valida com `aws-jwt-verify`. Grupo `admin` libera `POST /db/stop`.
